...
@Injectable() export class AckCache extends AckOffline{
  ...
  validate(data: any, config: cacheModel) {
  ...
  */
  ...
  selfDestructData(name: string, data: cacheModel){
    if( this.dataDestructReady(data) )return this.clear(name)
    return Promise.resolve()
  }

  dataDestructReady(data: cacheModel){
  ...
  dataOptionsCache(allCache: any, options:cacheModel=<cacheModel>{}, cache:any){
