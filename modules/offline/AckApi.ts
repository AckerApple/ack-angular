...
@Injectable() export class AckApi{
  ...
  */
  ...
  getCacheByNamedRequest(
    ...
    return this.AckCache.get((offlineModel as any).name, offlineModel)
    .then(routes=>{
      routes = routes || {}
  ...
  requestOfflineModel(
    ...
    this.AckQue.paramHandler((offlineModel as any).name,config=>this._fetch(config))
  ...
  processCacheGet(
    ...
    return this.AckCache.cacheToReturn((offlineModel as any).name, cache, offlineModel)
    .then((rtn: any)=>{
  ...
  postRequestFail(
    e: Record<string, any>, request:httpOptions
  ):Promise<any>{
    ...
    if(tryAgainLater){
      ...
      const offlineModel = request.offlineModel
      return this.AckQue.set((offlineModel as any).name, requestSave)
  ...
  */
  ...
  requestResponseToCache(
    request: httpOptions, output: any
  ):Promise<any>{
    const cachename = request.offlineModel?.name || request.offlineModel as  string
