  ...
  */
  ...
  paramHandler(name:string, handler:(qued:any)=>any) {
    for(let x=this.handlers.length-1; x >= 0; --x){
      const handler = this.handlers[x] as any
  ...
  /** return functions */
  getQueHandlerByName(name: string){
  ...
  eachHandler(
    ...
    return (data: any) => this.handleQued(data, handler)
  }
